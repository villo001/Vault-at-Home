scriptName DefaultActivateLinkedRefOnActivate extends ObjectReference Default
...
Auto State PreActivation
	Event OnActivate(ObjectReference akActionRef)
		if (DefaultScriptFunctions.CheckForMatches(RefToCheck = akActionRef, ReferenceArray = ActivatedByReferences, AliasArray = ActivatedByAliases, FactionArray = ActivatedByFactions, PlayerCheckOverride = PlayerActivatedOnly) == TRUE)
			DefaultScriptFunctions.DefaultScriptTrace(akactionRef + " is activating " + GetLinkedRef(LinkedRefKeyword) + " by way of " + self, ShowTraces)
			Self.GetLinkedRef(LinkedRefKeyword).Activate(akActionRef)
			if DoOnce
