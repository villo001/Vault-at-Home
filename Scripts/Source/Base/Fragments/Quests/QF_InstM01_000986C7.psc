...
Scriptname Fragments:Quests:QF_InstM01_000986C7 Extends Quest Hidden Const
...
Alias_Bill.GetActorRef().SetValue(Aggression, 2)
...
Alias_Bill.GetActorRef().SetCrimeFaction(None)
...
;BEGIN CODE
;Set when Bill dies (via a script on his alias)

;If the player killed Bill at or later than stage 800, it means he died while threatening the family. Set stage 1100.
;Otherwise, the player murdered Bill before he threatened the family. Set stage 91.
...
;BEGIN CODE
;Set when Roger dies, which can only happen if the player murders him (protected)
...
;BEGIN CODE
;Set if the player caused the mission to fail by murdering someone at Warwick Homestead
;Note the player will NOT trigger this if they Kill Bill Sutton during the standoff (ie, Stage 800 was set)
;The player will also not trigger this by killing Cedric Hopton at the Third Rail
...
;Set from Holdren's greeting subsequent scene in DialogueInstitute if the player chooses the ? option
...
;Add the Warwick Homestead map marker
NutIslandMapMarkerRef.AddToMap()

;Add seeds to player's inventory
Game.GetPlayer().AddItem(Alias_Seeds.GetReference(), 1)
;END CODE
EndFunction
;END FRAGMENT
...
;BEGIN CODE
;Remove the seeds from the player's inventory
Game.GetPlayer().RemoveItem(Alias_Seeds.GetReference(), 1)
;END CODE
EndFunction
;END FRAGMENT
...
;Give small xp reward NO LONGER USED
...
;BEGIN CODE
;Roger Warwick sends the player to search for Cedric

;Remove Cedric from the CedricEssential alias, so he will no longer be essential
Alias_CedricEssential.Clear()
...
;Set if the player successfully talks Bill Sutton out of his plan and gets him to leave peacefully
...
;BEGIN CODE
;Set at the end of InstM01IsaacSceneFailed
;Conditions hellos on Isaac and Clayton in DialogueInstitute
...
ObjectReference Property NutIslandMapMarkerRef Auto Const

ReferenceAlias Property Alias_CedricEssential Auto Const
